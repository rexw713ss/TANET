...
檢索增強生成（Retrieval-Augmented Generation, RAG）會將網頁、文件或工具回傳內容送入大型語言模型，因此攻擊者可在外部資料中嵌入間接提示注入（Indirect Prompt Injection, IPI），誘使模型偏離原始任務。本研究提出不修改生成模型權重的雙層防禦：第一層使用 multilingual-E5 意圖偏移與指令密度形成語意風險分數（Semantic Risk Score, SRS），第二層只審查候選指令或風險邊界樣本，並輸出結構化、可稽核的判定。兩個獨立封存且 context-disjoint 的 BIPIA 五任務 replication 合計 200 rows；pooled no-defense ASR 為 23.0%，完整方法為 1.0%，配對差為 −22 percentage points（95% CI [−29, −15]；exact McNemar `p=4.77×10⁻⁷`），benign utility preservation 為 98.33%，benign block rate 為 3.0%。相較每筆都使用相同 Tier-2 Judge 的 v4 baseline，候選路由減少 74% Tier-2 calls，平均 detector latency 降為約四分之一；always-on Judge 雖達 0% ASR，卻只有 94.68% utility preservation 與 6% benign block。380 筆 validation 消融顯示完整 SRS 的 AUPRC 為 0.885，高於 instruction-only 的 0.845、intent-only 的 0.724 與 pressure-only 的 0.750。外部穩定性測試則發現 InjecAgent base 與 HouYi 變體仍大量通過 Gate，顯示方法對明顯覆寫句型有效，但對隱晦工具動作與句型變體的跨 benchmark 泛化仍有限。
...
| 研究問題 | 證據 | 目前答案 |
|---|---|---|
| RQ1：雙層防禦能否降低 BIPIA 下游攻擊成功？ | 兩輪共 100 個 context-disjoint malicious pairs | 可以；pooled ASR 23% → 1%，兩輪配對差皆 −22 points |
| RQ2：安全改善是否保留正常任務效用？ | 兩輪共 100 benign pairs、replication×task bootstrap | Pooled preservation 98.33%；效用差 CI 含 0，benign block 3% |
...
| Full Tier-2 Judge | **0% [0, 7.13]** | 0.5947 [0.4964, 0.6842] | 94.68% | 6% | 96% | 100% |
...
| No-defense ASR | 24% [14.30%, 37.41%] | 22% [12.75%, 35.24%] | 23% [15.84%, 32.15%] |
| Two-stage ASR | 2% [0.35%, 10.50%] | 0% [0%, 7.13%] | **1% [0.18%, 5.45%]** |
| Paired ASR difference | −22 [−34, −12] | −22 [−32, −12] | **−22 [−29, −15]** |
| Exact McNemar p | 0.0009766 | 0.0009766 | **4.77×10⁻⁷** |
| Utility preservation | 96.84% | 99.75% | **98.33%** |
| Benign block | 4% | 2% | 3% [1.03%, 8.45%] |
| Tier-2 trigger | 26% | 24% | 25% [19.51%, 31.43%] |

Pooled bootstrap 使用 `replication × task` strata，沒有重新校準 Gate。兩個 replication 的 baseline ASR 接近（24% vs 22%），且配對改善都正好是 −22 points，支持安全效果可重現。Pooled benign utility 差為 −0.01075（95% CI [−0.03998, 0.01577]），仍不能宣稱效用顯著上升或下降。
...
| Full Tier-2 Judge | 14,781.29 ms | 14,307.41 ms | 19,023.99 ms |

Two-stage 長尾來自 Tier-2 本地模型。相同 100 IDs 的 always-on baseline 證實候選路由將 Tier-2 trigger 由 100% 降至 26%（calls −74%），平均 detector latency 由 14.78 秒降至 3.84 秒，約為 full baseline 的四分之一。Always-on 的 ASR 由 2% 再降至 0%，但 benign block 由 4% 增至 6%，utility preservation 由 96.84% 降至 94.68%。兩次 run 是 sequential 而非 interleaved，cache/model residency 可能影響 latency，因此可宣稱觀察到約 3.85× mean latency ratio，不應把它寫成硬體無關的固定加速倍數。兩筆 schema-truncated JSON 僅以較長 `num_predict` 重判，repair provenance 已封存，其他 98 筆未重跑。
...
**主要成效。** 兩個 inference 前封存且 context-disjoint 的 BIPIA holdouts 共包含 100 個 malicious pairs。Pooled 無防禦 ASR 為 23.0%（95% CI [15.84%, 32.15%]），完整雙層方法為 1.0%（95% CI [0.18%, 5.45%]）。配對差為 −22 percentage points（95% CI [−29, −15]），22 個案例僅在無防禦時成功，沒有案例僅在雙層防禦時成功，exact McNemar test 為 `p=4.77×10⁻⁷`。v4 與 v5 的配對差均為 −22 points，支持效果可重現。

**可用性。** Pooled benign utility preservation 為 98.33%，benign block rate 為 3.0%。相對無防禦的配對效用差為 −0.01075（95% CI [−0.03998, 0.01577]），信賴區間包含 0。因此結果未提供明確的平均效用下降證據，但三筆正常內容被阻擋，顯示安全改善仍存在可量化的可用性成本。

**Full Tier-2 baseline。** 在相同 100 個 IDs 上，每筆均執行 v3 structured Judge 時，ASR 為 0%（95% CI [0%, 7.13%]），但 benign block rate 增至 6%，utility preservation 降至 94.68%。候選路由將 Tier-2 calls 減少 74%，平均 detector latency 從 14.78 秒降至 3.84 秒，同時維持 2% ASR。此結果支持雙層架構的主要價值是以小幅安全差距交換較低審查成本與較佳 benign utility。
