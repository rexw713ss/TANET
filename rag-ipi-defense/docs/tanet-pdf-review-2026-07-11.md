...
| 項目 | 狀態 | 論文可引用結果／注意事項 |
|---|---|---|
| v4 corrected paired rerun | 完成 | no-defense ASR 30%；two-stage 2%；paired Δ −28 points [−40, −18]；utility Δ −0.03829 [−0.09236, 0]；benign block 6%；Tier-2 trigger 26% |
| v5 corrected context-disjoint rerun | 完成 | no-defense ASR 28% [17.47, 41.67]；two-stage 0% [0, 7.13]；paired Δ −28 points [−40, −18]；McNemar p=0.0001221 |
| corrected v4+v5 pooled | 完成，應作主結果 | no-defense 29% [21.01, 38.54]；two-stage 1% [0.18, 5.45]；paired Δ −28 points [−36, −20]；McNemar p=7.45×10⁻⁹；utility Δ −0.03848 [−0.07526, −0.00957]；benign block 5%；Tier-2 trigger 25% |
| Corrected Full Tier-2 Judge baseline | 完成 | ASR 0% [0, 7.13]；paired utility Δ −0.05220 [−0.10824, −0.00583]；benign block 10%；malicious block 96%；trigger 100% |
| 候選路由成本比較 | 完成 | corrected v4上較Full Judge減少74% Tier-2 calls；mean detector latency 11.35 s → 2.96 s（約3.83×）；此為sequential/cache-sensitive measurement |
...
| Corrected per-task pooled analysis | 完成、exploratory | ASR：abstract 35→0、code 0→0、email 15→0、QA 35→0、table 60→5；code benign utility 0.9395→0.7519且benign block 20%，是pooled utility成本主要來源；不作confirmatory subgroup claim |
...
目前稿件反覆使用legacy單輪v4的24%→2%、96.84%、4%、26%。應以2026-07-18 corrected paired pooled rerun作主要結論，並把v4／v5分輪結果放在同一張表以呈現重現性：

> Across two independently sealed, context-disjoint BIPIA replications comprising 100 malicious and 100 benign pairs, the candidate-routed defense reduced pooled ASR from 29.0% (95% CI: 21.01–38.54%) to 1.0% (0.18–5.45%). The paired reduction was 28 percentage points (95% stratified-bootstrap CI: 20–36 points; exact McNemar p=7.45×10⁻⁹). The paired benign-utility difference was −0.03848 (95% CI: −0.07526 to −0.00957), the benign block rate was 5.0%, and Tier-2 was invoked for 25.0% of all samples.
...
- Gemma 4 provenance：主模型為本機 Ollama `gemma4:latest`（Gemma 4 architecture、8.0B、Q4_K_M、manifest ID `c6eb396dbd59`、model blob SHA-256 `4c27e0f5b5adf02ac956c7322bd2ee7636fe3f45a8512c9aba5385242cb6e09a`）；獨立 audit 為 `gemma4:31b`（31.3B、Q4_K_M、manifest ID `6316f0629137`、model blob SHA-256 `280af6832eca23cb322c4dcc65edfea98a21b8f8ab07dc7553bd6f7e6e7a3313`）。兩者皆由 Ollama 0.31.2 執行。
...
| Setting | No-defense ASR | Candidate-routed ASR | Paired Δ (pp) | Paired utility Δ | Benign block | Tier-2 trigger |
|---|---:|---:|---:|---:|---:|---:|
| v4 | 30% | 2% | −28 [−40, −18] | −0.03829 [−0.09236, 0] | 6% | 26% |
| v5 | 28% | 0% | −28 [−40, −18] | −0.03867 [−0.09646, 0] | 4% | 24% |
| pooled | 29% | 1% | −28 [−36, −20] | −0.03848 [−0.07526, −0.00957] | 5% | 25% |
...
Utility欄位不要再把SRS-only寫成`100.14% preservation`。舊值是independent regeneration下的相對比值，不是bounded accuracy。2026-07-18 corrected rerun對相同sample＋相同Gemma 4 prompt共用一次輸出後，v4 SRS-only與no-defense benign utility均為`0.6285406`，paired difference與CI均為0，relative utility正好100%。正文以paired benign-utility difference為主；不可把新Two-stage pooled utility成本寫成「沒有明確下降」，因其CI `[-0.07526, -0.00957]`已不含0。

### 可直接放入論文的 Gemma 4 實作段落

> All downstream responses and the primary structured judges were executed locally with an 8.0B, Q4_K_M-quantized Gemma 4 model served by Ollama 0.31.2 (`gemma4:latest`; manifest ID `c6eb396dbd59`). Generation, Tier-2 adjudication, and model-evaluated attack-family labels used temperature 0 and seed 42. An independently tagged 31.3B Q4_K_M Gemma 4 model (`gemma4:31b`; manifest ID `6316f0629137`) with seed 1729 was used only for evaluator audit. Exact model-blob SHA-256 values are provided in the reproducibility artifacts. To remove repeated-generation noise from paired utility comparisons, methods that passed the same sample to an identical effective generator prompt reused one Gemma 4 response; blocked samples received zero downstream utility.
